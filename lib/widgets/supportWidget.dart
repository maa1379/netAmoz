...
import 'package:ehyasalamat/Screen/SupportChatScreen.dart';
...
import 'package:ehyasalamat/Screen/SupportStepperScreen.dart';
...
class _SupportWidgetState extends State<SupportWidget> {
  ...
  SupportStepperController supportStepperController = Get.put(
      SupportStepperController());
  ...
  _searchFunction() {
    _filtered = [].obs;
    for (int i = 0; i <
        supportStepperController.supportTicketList.length; ++i) {
      var item = supportStepperController.supportTicketList[i];
      if (item.topic
          .toLowerCase()
        ...
        child: Obx(
              ...
              Column(
                children: [
                  ...
                  if (supportStepperController.supportTicketList.length == 0)
                    Padding(
      ...
      onTap: () {
        if (getProfileBlocInstance.profile.profileDone == false) {
          AlertHelpers.CheckProfileDialog(
            ...
            context: Get.context,
            func: () {
              NavHelper.pushR(Get.context, EditProfileScreen());
            },
          );
        } else {
          NavHelper.push(Get.context, SupportStepperScreen());
          print("profile ok");
        }
      },
      child: Container(
...
// color: Color(0xfff5f5f5),
    ...
    return Expanded(
      child: ListView.builder(
          ...
          itemCount: searchTextEditingController.text.isEmpty
              ? supportStepperController.supportTicketList.length
              : _filtered.length,
  ...
  Widget _buildListItem(BuildContext context, int index) {
    var item = supportStepperController.supportTicketList[index];
    return Obx(
          () {
        if (supportStepperController.supportTicketList.length == 0) {
        ...
        return GestureDetector(
          onTap: () {
            Get.to(SupportChatScreen(),
                arguments: {"supportChat_id": item.id.toString()});
          },
          child: Container(
